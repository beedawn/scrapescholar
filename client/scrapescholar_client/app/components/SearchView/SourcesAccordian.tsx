        ...
        <div id="accordion-color">
                <div key={index}>
                    <h2 id={`accordion-color-heading-${index + 1}`}>
                        <button
                            type="button"
                            className={`flex items-center  w-full p-3 font-medium text-gray-500 
                    ...
                    aria-labelledby={`accordion-color-heading-${index + 1}`}>
                        <div className="pl-5 dark:border-gray-700">
                            {databases.map((database, index) => (
                                ...
                                <input id={database} type="checkbox" name={database} checked={checkboxes[database]} 
                                onChange={(event)=>{handleCheckboxChange(event,database)}}/>
                                {database}</label></div>))}
                        </div>
                    </div>
                </div>
