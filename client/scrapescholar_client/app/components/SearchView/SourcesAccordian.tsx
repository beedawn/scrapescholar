...
const databases =["ScienceDirect","Scopus"];
    ...
    const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>, database:string)=>{
        ...
        <div id="accordion-color">
                ...
                <div key={index}>
                    <h2 id={`accordion-color-heading-${index + 1}`}>
                        <button
                            type="button"
                            className={`flex items-center  w-full p-3 font-medium text-gray-500  rounded-t-xl ${openIndex === index ? '' : ''}`}
                            onClick={() => toggleAccordion(index)}
                            aria-expanded={openIndex === index}
                            aria-controls={`accordion-color-body-${index + 1}`}
                        >
                            <svg className={`w-3 h-3 ${openIndex === index ? 'rotate-180' : 'rotate-90'}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
