...
from utils.auth import get_current_user  # Import the get_current_user utility
from app.models.user import User  # Assuming the User model is needed

router = APIRouter()

# Add a comment to an article
@router.post("/article/{article_id}", response_model=Comment, status_code=201)
def create_new_comment(article_id: int, comment: CommentCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    new_comment = create_comment(db, article_id=article_id, comment=comment, user_id=current_user.user_id)
    return new_comment

# Edit a comment
@router.put("/{comment_id}", response_model=Comment)
def update_existing_comment(comment_id: int, comment: CommentUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
