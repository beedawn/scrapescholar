# app/crud/rsearchquestionscore.py
...
def get_research_question_score(db: Session, research_question_score_id: int):
    score = db.query(ResearchQuestionScore).filter(ResearchQuestionScore.research_question_score_id == research_question_score_id).first()
    if not score:
