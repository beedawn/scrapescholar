...
# Function to get the current user based on the JWT token from a route
...
# Function to get the current user based on a token passed as a parameter (e.g., from a cookie)
async def get_current_user_no_route(db: Session, token: str):
    try:
        if DEBUG_SCRAPESCHOLAR:
            print(f"Decoding token: {token}")
