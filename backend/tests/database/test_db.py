...
def test_article_creation_with_existing_user_and_source(db: Session):
    ...
    article = Article(user_id=user.user_id, source_id=source.source_id, search_id=search.search_id,
...
def test_article_table_insert_with_new_data(db: Session):
    ...
    source_data = SourceCreate(name="New Sample Source", api_endpoint="http://newapi.example.com",
                               scrape_source_url="http://newscrape.example.com")
    source = create_source(db=db, source=source_data)

    search_data = SearchCreate(user_id=user.user_id, search_keywords=["sample", "keywords"],
                               title="Sample Search Title")
    search = create_search(db=db, search=search_data)

    article = Article(title="New Sample Article", source_id=source.source_id, search_id=search.search_id,
...
def test_full_search_insert_and_share(db: Session):
    ...
    article_1_data = ArticleCreate(source_id=source.source_id, search_id=search.search_id, title="Article 1",
                                   date=datetime.now().date())
    article_2_data = ArticleCreate(source_id=source.source_id, search_id=search.search_id, title="Article 2",
                                   date=datetime.now().date())
    ...
    search_share_data = SearchShareCreate(search_id=search.search_id, shared_with_user_id=new_user.user_id,
                                          shared_by_user_id=existing_user.user_id)
