...
def test_sciencedirect_response_has_title_and_link():
    response = client.get("/academic_data?keywords=test&academic_database=Science%20Direct")
...
def test_sciencedirect_empty_response_is_empty():
    response = client.get("/academic_data?keywords=abcdefg+AND+hijklmnop+AND+12345&academic_database=Science%20Direct")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
