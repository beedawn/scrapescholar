...
def test_mock_scopus(setup_mock_scopus):
    ...
    result_articles, article_id, status_code = request_data(keywords=keywords, id=id, key=key, subject=subject, min_year=min_year)
    ...
    assert result_articles[
               0].title == "A Systematic and Meta-Analysis Review of Augmented Reality Interventions for Individuals with Autism Spectrum Disorder"
    assert result_articles[
               0].link == "https://www.scopus.com/inward/record.uri?partnerID=HzOxMe3b&scp=85206900643&origin=inward"
